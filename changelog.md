...
* **🔧 Risoluzione Completa Visualizzazione & Harvesting Port Forwarding:**
  * **Risolto Bug Critico di Routing FastAPI (Shadowing Endpoint Rules):** Risolta la collisione di routing in `devices.py`: la rotta generica catch-all `@router.get("/{device_id_or_mac:path}")` era definita prima di `@router.get("/{mac_address}/rules")`, intercettando qualsiasi chiamata verso `.../rules` e restituendo un oggetto nullo che azzerava la lista delle regole nel modale. Riposizionata la rotta catch-all alla fine del router.
  * **Arricchimento Bidirezionale Dispositivo (`dev_map`):** Risolto il caso reale in cui le regole create dall'app ufficiale eero collegano solo la risorsa `device` senza includere un campo `ip` esplicito nel payload JSON: la dashboard ora incrocia istantaneamente MAC e IP del dispositivo registrato in cache, popolando correttamente le regole.
  ...
  * Implementato l'harvesting ricorsivo delle regole di inoltro porte sia da endpoint dedicati (`/forwards` e `/port_forwards`) sia annidate all'interno di ciascun oggetto prenotazione DHCP (`r["forwards"]` o `r["port_forwards"]`), con fallback sulle proprietà di rete `/networks/{id}`, deduplicazione automatica e arricchimento tramite `res_map` e `dev_map`.
  * Ottimizzato il matching delle regole nella scheda del dispositivo (`/api/devices/{mac}/rules`) verificando correlazione multi-criterio: set `target_ips` (IP attivo, IP prenotato, live IP puliti da porte o CIDR), MAC address e puntatori risorsa eero.
