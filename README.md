...
* **📊 Network Statistics, Mesh Load & ISP SLA Analytics (v1.5.0):** Dedicated analytics view featuring live frequency distribution (2.4/5/6 GHz & Wired), mesh node load balancing, client device categories, OUI hardware vendor recognition, 7d/30d ISP SLA line quality trends with dual-axis throughput/ping charts, dark distribution chart tooltips with device name previews, and an RFC 4180 CSV / structured JSON Data Export Center for client devices, speedtests, signal samples, and data usage.
* **🔒 Security Hardening & Vulnerability Remediation (v1.5.0 - 4 GHSA Advisories):** Comprehensive security protection including CORS/CSRF middleware with `CORS_ORIGINS`, automatic removal of Wi-Fi credentials from the public network overview, interactive Swagger/OpenAPI documentation toggle (`API_DOCS`), and `0600` owner-only file permissions on `session.json`.
* **🌐 Dual-Stack IPv6 Visibility & Instant Search (v1.5.0 - Issue #43):** Full visibility and instant search for both Global Unicast / SLAAC and Link-Local (`fe80::`) IPv6 addresses with 1-click clipboard copy, and unobtrusive device table badge.
* **🎨 Windows 11 Fluent Design & Dual Theme Engine (v1.4.0):** Native Windows 11 Mica & Acrylic materials, Segoe UI Variable font stack, refined layered borders and shadows, plus a 3-state header toggle (☀️ Light / 🌙 Dark / 💻 System Auto) with zero-flicker loading and real-time adaptive Chart.js themes.
* **📐 Collapsible Left Sidebar Navigation & Relocated Controls (v1.4.0):** Fluent-style vertical navigation sidebar replacing top horizontal tabs with smooth expanding (`256px` / `w-64`) and collapsing (`68px` / `w-[68px]`) animations, perfect 44x44px icon centering in collapsed mode, top-right device badge placement, `localStorage` state persistence, dedicated footer toggle buttons (`<<` / `>>`) plus header hamburger button, relocated quick controls (**Gaming Focus Mode** & **Demo Mode**), and a vibrant emerald green high-visibility indicator when Demo Mode is active.
...
* **📱 Dynamic Dual-Theme Guest Wi-Fi QR Code:** Scannable Wi-Fi QR Code generator (`WIFI:S:...;T:WPA;P:...;;`) with pure white background for Light Mode and dark slate for Dark Mode, instant theme synchronization, one-click enable/disable, and secure password generation.
...
<p align="center">
  <img src="docs/screenshots/dashboard_overview.png" alt="Panoramica eero Dashboard" width="100%" style="border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);" />
</p>
...
* **📊 Statistiche di Rete, Carico Mesh & Diagnostica SLA ISP (v1.5.0):** Nuova sezione Analytics con ripartizione frequenze radio (2.4/5/6 GHz e Cablato), bilanciamento carico nodi mesh, categorie apparati, fingerprinting vendor OUI, monitoraggio SLA ISP a 7/30 giorni con grafici throughput/ping a doppio asse Y, tooltip scuri interattivi con anteprima dispositivi e Centro Esportazione Dati aperto in standard RFC 4180 CSV e JSON strutturato.
* **🔒 Hardening di Sicurezza & Vulnerability Remediation (v1.5.0 - 4 Advisory GHSA):** Protezione di sicurezza avanzata con middleware anti-CSRF e controllo `CORS_ORIGINS`, sanitizzazione automatica delle password Wi-Fi dall'overview pubblico, disattivazione di default della documentazione interattiva `/docs` (`API_DOCS`) e permessi restrittivi `0600` per il token cloud in `session.json`.
* **🌐 Visibilità & Ricerca Indirizzi IPv6 Dispositivi (v1.5.0 - Issue #43):** Piena visibilità e ricerca istantanea sia per indirizzi IPv6 Global Unicast / SLAAC che Link-Local (`fe80::`), con pulsante di copia negli appunti con 1 clic nel modale apparato e badge discreto nella tabella dispositivi.
* **🎨 Design Windows 11 Fluent & Dual Theme Engine (v1.4.0):** Restyling estetico con effetti Mica/Acrylic, Segoe UI Variable font stack, bordi stratificati e selettore a 3 stati (☀️ Chiaro / 🌙 Scuro / 💻 Sistema Auto) con zero sfarfallio al caricamento e ricolorazione dinamica dei grafici Chart.js.
* **📐 Navigazione a Barra Laterale Collassabile & Controlli Rapidi (v1.4.0):** Menu di navigazione verticale a sinistra in stile Windows 11 Fluent con modalità espansa (`256px`) e collassata (`68px`), animazioni fluide con margine principale dinamico, perfetta centratura geometrica 44x44px delle icone in modalità compatta con micro-badge d'angolo, salvataggio dello stato in `localStorage`, pulsanti dedicati di espansione/collasso (`<<` / `>>` e hamburger nell'header), controlli rapidi **Gaming Focus Mode** e **Demo Mode** integrati nella barra e indicatore grafico verde smeraldo brillante ad alta visibilità a simulazione attiva.
* **❤️ Diagnostica Avanzata & Breakdown Health Score a 4 Pilastri (v1.4.0 - Issue #15):** Badge circolare dell'Health Score interattivo e cliccabile con finestra modale esplicativa basata su 4 pilastri (**Topologia Mesh, Gateway WAN, Qualità Segnale Client RSSI, Bilanciamento Canali**), analisi puntuale delle penalità attive (nodi offline, client degradati, latenza) e raccomandazioni intelligenti per massimizzare la stabilità di rete.
* **🛡️ Multi-Engine DNS Synchronizer Suite (v1.4.0):** Sincronizzazione automatica e simultanea verso molteplici server DNS eterogenei (es. 2 AdGuard Home + 1 Pi-hole / Technitium) con test individuali e sync globale.
* **🔄 Auto-Update Docker in-App a 1-Clic & Controllo Versioni (v1.4.0):** Verifica automatica e manuale di nuove release su Docker Hub e GitHub, badge di notifica animato nell'header e aggiornamento istantaneo del container in 1 clic tramite Docker socket (`/var/run/docker.sock`), webhook Watchtower o comando assistito per terminale.
* **📶 Qualità Segnale Wi-Fi & Copertura Mesh (v1.4.0):** Storicizzazione continua su database SQLite (`device_signal_history`) dei valori RSSI (dBm), bande e bitrate dei dispositivi wireless, KPI del segnale medio di casa, grafico temporale Chart.js interattivo (24h/7g) e Weak Signal Watchlist con suggerimenti di posizionamento nodi mesh.
...
* **📱 Smart Guest Wi-Fi con QR Code Dual-Theme:** Generatore automatico di QR Code standard Wi-Fi (`WIFI:S:...;T:WPA;P:...;;`) con sfondo bianco puro ottimizzato per la modalità chiara (Light Mode) e ardesia per la modalità scura (Dark Mode), sincronizzazione istantanea al cambio tema, attivazione/disattivazione con un clic e generazione sicura di password.
